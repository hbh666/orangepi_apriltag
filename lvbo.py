...
# 0 表示每次有效定位都发送；大于 0 时才按最小间隔限速。
SEND_INTERVAL = 0.0
PRINT_INTERVAL = 1.0
SEND_GAP_WARN_MS = 60.0
SEND_GAP_BAD_MS = 80.0
POSE_FILTER_ENABLE = True
POSE_FILTER_ALPHA_POS = 0.35
POSE_FILTER_ALPHA_YAW = 0.35
POSE_FILTER_RESET_MISS = 5
...
class PreviewHandler(BaseHTTPRequestHandler):
    """Very small MJPEG server using only Python stdlib."""
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            html = (
                b"<!doctype html><html><head><title>AprilTag Tracker</title>"
                b"<style>html,body{margin:0;width:100%;height:100%;"
                b"background:#050505;display:grid;place-items:center}"
                b"img{max-width:100vw;max-height:100vh;object-fit:contain}"
                b"</style></head><body><img src='/video_feed'></body></html>"
            )
...
def start_web_preview():
    if not ENABLE_WEB_PREVIEW:
        return
    server = ThreadingHTTPServer((WEB_PREVIEW_HOST, WEB_PREVIEW_PORT), PreviewHandler)
    server.daemon_threads = True
    print(f"Web preview: http://{get_preview_host()}:{WEB_PREVIEW_PORT}/", flush=True)
    server.serve_forever()


def get_preview_host():
    if WEB_PREVIEW_HOST not in ("0.0.0.0", "::"):
        return WEB_PREVIEW_HOST
...
def smooth_angle_deg(prev_angle, current_angle, alpha):
    delta = wrap_angle_deg(float(current_angle) - float(prev_angle))
    return wrap_angle_deg(float(prev_angle) + float(alpha) * delta)
...
def build_preview_frame(
    ...
    cv2.putText(
        display_frame,
        "SYSTEM READY" if is_calibrated else f"Calibrating: Found {len(base_cache)}/4",
        (14, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (0, 255, 0) if is_calibrated else (0, 165, 255),
        2,
    )
    draw_tag_markers(
        display_frame,
    ...
    cv2.putText(
        display_frame,
        f"FPS: {fps:.1f} CAM:{camera_capture_fps:.1f}",
        (18, 75),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (0, 255, 255),
    ...
    if last_car_x_cm is not None and last_car_y_cm is not None:
        cv2.putText(
            display_frame,
            f"X:{int(last_car_x_cm)}cm Y:{int(last_car_y_cm)}cm",
            (300, 30),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (0, 255, 255),
...
print(
    f"Display: {'on' if ENABLE_DISPLAY else 'off'} "
    f"{DISPLAY_W}x{DISPLAY_H} @ {1.0 / DISPLAY_INTERVAL:.0f}fps"
)
print(f"OpenCV threads: {CV_THREADS}")
if ENABLE_WEB_PREVIEW:
    print(f"Web preview starting on port {WEB_PREVIEW_PORT}...")
    threading.Thread(target=start_web_preview, daemon=True).start()
...
last_car_x_cm = None
last_car_y_cm = None
filtered_car_x_cm = None
filtered_car_y_cm = None
filtered_yaw = None
last_scan_mode = "FULL"
...
send_window_t0 = last_print_time
send_window_count = 0
send_gap_count = 0
send_gap_sum_ms = 0.0
send_gap_min_ms = None
send_gap_max_ms = 0.0
send_gap_warn_count = 0
send_gap_bad_count = 0
send_age_sum_ms = 0.0
send_age_max_ms = 0.0
send_roi_count = 0
send_full_count = 0
send_no_car_count = 0
send_no_phys_count = 0
    ...
    while True:
        ...
        if not is_calibrated:
                ...
                for i in range(len(ids_car)):
                    if ids_car[i][0] == 1:
                        ...
                        if phys_coord:
                            ...
                            raw_car_x_cm, raw_car_y_cm = phys_coord
                            ...
                            if (
                                POSE_FILTER_ENABLE and
                                filtered_car_x_cm is not None and
                                filtered_car_y_cm is not None and
                                filtered_yaw is not None
                            ):
                                filtered_car_x_cm = (
                                    filtered_car_x_cm +
                                    POSE_FILTER_ALPHA_POS * (raw_car_x_cm - filtered_car_x_cm)
                                )
                                filtered_car_y_cm = (
                                    filtered_car_y_cm +
                                    POSE_FILTER_ALPHA_POS * (raw_car_y_cm - filtered_car_y_cm)
                                )
                                filtered_yaw = smooth_angle_deg(
                                    filtered_yaw,
                                    raw_yaw,
                                    POSE_FILTER_ALPHA_YAW,
                                )
                            else:
                                filtered_car_x_cm = raw_car_x_cm
                                filtered_car_y_cm = raw_car_y_cm
                                filtered_yaw = raw_yaw

                            car_x_cm = filtered_car_x_cm
                            car_y_cm = filtered_car_y_cm
                            real_yaw = round(filtered_yaw, 2)
                            ...
                            if SEND_INTERVAL <= 0 or now - last_send_time >= SEND_INTERVAL:
                                # SEND_INTERVAL 为 0 时，每次有效识别都直接发送。
                                payload = {
                                    "type": "robot_position",
                                    "pos": [x_m, 0.00, z_m],
                                ...
                                try:
                                    ...
                                    send_window_count += 1
                                    send_age_ms = (actual_send_time - frame_time) * 1000.0
                                    send_age_sum_ms += send_age_ms
                                    send_age_max_ms = max(send_age_max_ms, send_age_ms)
                                    if scan_mode == "FULL":
                                        send_full_count += 1
                                    else:
                                        send_roi_count += 1

                                    if last_actual_send_time is not None:
                                        send_gap_ms = (actual_send_time - last_actual_send_time) * 1000.0
                                        send_gap_count += 1
                                        send_gap_sum_ms += send_gap_ms
                                        send_gap_max_ms = max(send_gap_max_ms, send_gap_ms)
                                        if send_gap_min_ms is None:
                                            send_gap_min_ms = send_gap_ms
                                        else:
                                            send_gap_min_ms = min(send_gap_min_ms, send_gap_ms)
                                        if send_gap_ms >= SEND_GAP_WARN_MS:
                                            send_gap_warn_count += 1
                                        if send_gap_ms >= SEND_GAP_BAD_MS:
                                            send_gap_bad_count += 1
                                    last_actual_send_time = actual_send_time

                                    if actual_send_time - last_print_time >= PRINT_INTERVAL:
                                        send_elapsed = max(actual_send_time - send_window_t0, 1e-6)
                                        send_fps = send_window_count / send_elapsed
                                        gap_avg_ms = (
                                            send_gap_sum_ms / send_gap_count
                                            if send_gap_count > 0 else 0.0
                                        )
                                        gap_min_text = (
                                            "--" if send_gap_min_ms is None
                                            else f"{send_gap_min_ms:.0f}"
                                        )
                                        gap_avg_text = (
                                            "--" if send_gap_count == 0
                                            else f"{gap_avg_ms:.0f}"
                                        )
                                        gap_max_text = (
                                            "--" if send_gap_count == 0
                                            else f"{send_gap_max_ms:.0f}"
                                        )
                                        age_avg_ms = send_age_sum_ms / max(1, send_window_count)
                                        print(
                                            f"send: X:{x_m}m Z:{z_m}m "
                                            f"Yaw:{real_yaw} AlgFPS:{fps:.1f} "
                                            f"CAM:{camera.capture_fps:.1f} "
                                            f"Raw:{raw_car_x_cm / 100.0:.3f}/{raw_car_y_cm / 100.0:.3f}/{raw_yaw:.1f} "
                                            f"SendFPS:{send_fps:.1f} "
                                            f"Gap(ms):{gap_min_text}/{gap_avg_text}/{gap_max_text} "
                                            f">60:{send_gap_warn_count} >80:{send_gap_bad_count} "
                                            f"ReadDelay:{delay_ms:.0f}ms "
                                            f"Age:{age_avg_ms:.0f}/{send_age_max_ms:.0f}ms "
                                            f"Mode:{scan_mode} ROI/FULL:{send_roi_count}/{send_full_count} "
                                            f"Miss car/phys:{send_no_car_count}/{send_no_phys_count} "
                                            f"ROI:{car_roi}"
                                        )
                                        last_print_time = actual_send_time
                                        send_window_t0 = actual_send_time
                                        send_window_count = 0
                                        send_gap_count = 0
                                        send_gap_sum_ms = 0.0
                                        send_gap_min_ms = None
            ...
            if not found_car:
                send_no_car_count += 1
                lost_count += 1
                if lost_count >= POSE_FILTER_RESET_MISS:
                    filtered_car_x_cm = None
                    filtered_car_y_cm = None
                    filtered_yaw = None
        ...
        if (ENABLE_DISPLAY or ENABLE_WEB_PREVIEW) and now - last_display_time >= DISPLAY_INTERVAL:
            # 预览可以降分辨率和降刷新率，检测仍然使用原始高分辨率帧。
            display_frame = build_preview_frame(
                frame,
                is_calibrated,
                base_cache,
                last_base_corners,
            ...
            if ENABLE_WEB_PREVIEW:
                ...
                if ret:
                    with preview_lock:
                        latest_preview_jpeg = buffer.tobytes()

            if ENABLE_DISPLAY:
                cv2.imshow("Auto BEV Tracker & Radar", display_frame)
            last_display_time = now

        key = cv2.waitKey(1) & 0xFF if ENABLE_DISPLAY else 255
